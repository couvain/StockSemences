...
public class CMouvementNbPlants extends CMouvement 
{
	//******************************************************************************************
	//
	//
	//
	//******************************************************************************************
	public int get_mvt_nb_plants() 
	{
		return m_mvt_nb_plants;
	}
	...
	//
	//******************************************************************************************
	public void set_mvt_nb_plants(int p_mvt_nb_plants) 
	{
		this.m_mvt_nb_plants = p_mvt_nb_plants;
	}
	...
	public void bd_insert() 
	{
		...
		try 
		{
            ...
            pst.setInt(3, this.m_mvt_nb_plants);
	...
	//
	//******************************************************************************************
	static public LinkedList<CMouvementNbPlants> bd_select_date(int p_id_plante, long p_date_peremption, CDate p_date_debut, CDate p_date_fin) 
	{
		ResultSet   rs = null;
		LinkedList<CMouvementNbPlants> l_liste_mouvements = new LinkedList<CMouvementNbPlants>();
		CMouvementNbPlants l_objet_mouvement = new CMouvementNbPlants();
		int i;
		...
		try 
		{
			...
			if (rowCount > 0)
			{
				...
				for (i=1;i<=rowCount;i++)
				{
					...
					l_objet_mouvement = new CMouvementNbPlants();
		...
		catch (SQLException sqle) 
		{
			...
			l_message = "CMouvementNbPlants.bd_select_date_racine() [p_id_plante="+p_id_plante+", date_debut="+p_date_debut.toString() +", date_fin="+p_date_fin.toString()+ "]-> "+sqle.getMessage();
			JOptionPane.showMessageDialog(null, l_message);
		    System.err.println(l_message);
		    System.exit(1);
		}	
	...
	public String bd_toString() 
	{
		...
		{
			l_resultat = this.m_mvt_date.toStringFR() + " sortie de " + this.m_mvt_nb_plants + " plants de " +l_plante.get_pla_nom();
		}
		else
		{
			l_resultat = this.m_mvt_date.toStringFR() + " entrée de " + this.m_mvt_nb_plants + " plants de " +l_plante.get_pla_nom();
		}
