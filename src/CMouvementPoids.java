...
public class CMouvementPoids  extends CMouvement
{
	//******************************************************************************************
	//
	//
	//
	//******************************************************************************************
	public int get_mvt_poids_grammes() 
	{
		return m_mvt_poids_grammes;
	}
	...
	//
	//******************************************************************************************
	public int get_mvt_poids_grammes_sens() 
	{
		if (this.m_mvt_sens == CSensMouvement.MOUVEMENT_AJOUT_STOCK())
		{
			return m_mvt_poids_grammes;
		}
		else if (this.m_mvt_sens == CSensMouvement.MOUVEMENT_RETRAIT_STOCK())
		{
			return -m_mvt_poids_grammes;
		}
		else
	...
	//
	//******************************************************************************************
	public void set_mvt_poids_grammes(int p_mvt_poids_grammes) 
	{
		this.m_mvt_poids_grammes = p_mvt_poids_grammes;
	}
	...
	public void bd_insert() 
	{
		...
		try 
		{
            ...
            pst.setInt(3, this.m_mvt_poids_grammes);
		...
		catch (SQLException sqle) 
		{
			...
			l_message = this.getClass().getName()+".bd_insert(m_mvt_pla_id=" + this.m_mvt_pla_id + ") -> " + sqle.getMessage();
	...
	//
	//******************************************************************************************
	static public LinkedList<CMouvementPoids> bd_select_date(int p_id_plante, long p_date_peremption, CDate p_date_debut, CDate p_date_fin) 
	{
		ResultSet   rs = null;
		LinkedList<CMouvementPoids> l_liste_mouvements = new LinkedList<CMouvementPoids>();
		CMouvementPoids l_objet_mouvement = new CMouvementPoids();
		int i;
		...
		try 
		{
			...
			if (rowCount > 0)
			{
				...
				for (i=1;i<=rowCount;i++)
				{
					...
					l_objet_mouvement = new CMouvementPoids();
		...
		catch (SQLException sqle) 
		{
			...
			l_message = "CMouvementPoids.bd_select_date_racine() [p_id_plante="+p_id_plante+", date_debut="+p_date_debut.toString() +", date_fin="+p_date_fin.toString()+ "]-> "+sqle.getMessage();
			JOptionPane.showMessageDialog(null, l_message);
		    System.err.println(l_message);
		    System.exit(1);
		}	
	...
	public String bd_toString() 
	{
		...
		{
			l_resultat = this.m_mvt_date.toStringFR() + " sortie de " + this.m_mvt_poids_grammes + " grammes de " +l_plante.get_pla_nom();
		}
		else
		{
			l_resultat = this.m_mvt_date.toStringFR() + " entrée de " + this.m_mvt_poids_grammes + " grammes de " +l_plante.get_pla_nom();
		}
